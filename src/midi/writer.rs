...
pub trait ToTimedMidiBytes {
    fn try_convert(&self, curr: u32) -> Result<(u32, Vec<u8>), String>;
}
...
fn try_string_to_variable_bytes(str: &str) -> Result<Vec<u8>, String> {
    let mut chars: Vec<u8> = str.chars().flat_map(|c| c.to_string().into_bytes()).collect();
...
fn try_metadata_to_midi_bytes(d: &MetadataData) -> Result<Vec<u8>, String> {
    match d {
        ...
        MetadataData::Tempo(f32) => {
            ...
            ret.append(&mut try_number_to_fixed_bytes((6e7 / f32).round() as u32, 3)?);
...
mod tests {
    ...
    fn test_try_number_to_fixed_bytes() {
        ...
        assert_eq!(try_number_to_fixed_bytes(0xffff, 2), Ok(vec![ 0xff, 0xff ]));
        assert_eq!(try_number_to_fixed_bytes(0x010000, 3), Ok(vec![ 0x01, 0x00, 0x00 ]));
        assert_eq!(try_number_to_fixed_bytes(0xffffff, 3), Ok(vec![ 0xff, 0xff, 0xff ]));
        assert_eq!(try_number_to_fixed_bytes(0x01000000, 4), Ok(vec![ 0x01, 0x00, 0x00, 0x00 ]));
        assert_eq!(try_number_to_fixed_bytes(0xffffffff, 4), Ok(vec![ 0xff, 0xff, 0xff, 0xff ]));
        assert_eq!(try_number_to_fixed_bytes(0x03f62ab3, 4), Ok(vec![ 0x03, 0xf6, 0x2a, 0xb3 ]));
    }
    ...
    fn test_try_number_to_variable_bytes() {
        ...
        assert_eq!(try_number_to_variable_bytes(0x4000), Ok(vec![0x81,0x80,0x00]));
        assert_eq!(try_number_to_variable_bytes(0x1fffff), Ok(vec![0xff,0xff,0x7f]));
        assert_eq!(try_number_to_variable_bytes(0x200000), Ok(vec![0x81,0x80,0x80,0x00]));
        assert_eq!(try_number_to_variable_bytes(0x0fffffff), Ok(vec![0xff,0xff,0xff,0x7f]));
        assert_eq!(try_number_to_variable_bytes(0x03f62ab3), Ok(vec![0x9f,0xd8,0xd5,0x33]));
    }
}